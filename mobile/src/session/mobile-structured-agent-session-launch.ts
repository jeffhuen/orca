...
export async function createMobileStructuredAgentSession(
  ...
  if (!response.ok) {
    ...
    if (response.error.code === 'agent_session_operation_unknown') {
      return unknownCreateResult(agent, new Error(response.error.message))
    }
    return { kind: 'failed', message: response.error.message || failedMessage(agent) }
  }
  ...
  if (!result.ok) {
    ...
    if (result.refusal.code === 'agent_session_operation_unknown') {
      return unknownCreateResult(agent, new Error(result.refusal.message))
    }
    return { kind: 'failed', message: result.refusal.message || failedMessage(agent) }
  }
  if (
    !result.value ||
