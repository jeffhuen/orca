...
// re-written to disk each time. Remembering what this process last wrote at each path turns those
// into no-ops. Entries are dropped on delete and on write failure; the directory is Orca-owned, so
// nothing else replaces a file behind the cache.
...
function rememberWrittenSnapshotDigest(path: string, digest: string): void {
  ...
  if (writtenSnapshotDigestByPath.size > MAX_TRACKED_SNAPSHOT_DIGESTS) {
    const [oldest] = writtenSnapshotDigestByPath.keys()
